  ...
  return (
    <section className="function-lab" aria-labelledby="function-lab-title">
      <div className="function-lab-heading">
        <div><span className="eyebrow">SIMULAÇÃO GUIADA</span><h2 id="function-lab-title">Siga o valor até ele voltar pelo return</h2></div>
      ...
      <div className="function-controls">
        ...
        <div className="prediction-group" role="group" aria-label="Previsão do retorno">
          <span>Antes de avançar, qual valor volta?</span>
          <div>
            <button type="button" aria-pressed={prediction === true} className={prediction === true ? "selected" : ""} onClick={() => { setPrediction(true); setChecked(false); }}>true</button>
            <button type="button" aria-pressed={prediction === false} className={prediction === false ? "selected" : ""} onClick={() => { setPrediction(false); setChecked(false); }}>false</button>
      ...
      <div className="function-flow" aria-label="Fluxo da execução">
        {trace.map((item, index) => <div className="function-flow-piece" key={item.label}>
          <button type="button" aria-current={index === step ? "step" : undefined} className={`${index === step ? "active" : ""} ${index < step ? "visited" : ""}`} onClick={() => setStep(index)}>
